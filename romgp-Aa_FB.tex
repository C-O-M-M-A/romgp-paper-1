...
\begin{document}
    ...
    \section{Introduction} \label{sec:Intro}
        ...
        On the other hand Minimum Average Variance Estimation (MAVE) proposed by Xia et al \cite{Xia.etal2002} effectively uses a variance-based sensitivity (statistical independence) as the measure of input relevance. The method therefore bears some similarity to our own, but relies on local linear kernels, whose bandwidth must be chosen carefully.\todo{To me this seems a strange jump. From the previous paragraph to this next one. It goes GSA, then ROMs then back to GSA.}
    ...
    \section{Review of Gaussian Processes and Global Sensitivity Analysis} \label{sec:TheoryReview}
        ...
        \subsection{Global Sensitivity Analysis}
            ...
            \end{equation*}
            has a variation (over $\vr{x} \in \st{R}^{M}$) which can be apportioned by Sobol' index\todo{I personally, think the second equation I have inserted is tidier}
            \begin{equation*}
    ...
    \section{Methodology} \label{sec:Method}
        ...
        \subsection{Gaussian Process Surrogate} \label{sub:Method:GP}
           \todo{I would think it'd be easier to keep all the GP stuff in Section 2. Then here it can be much smaller explicitly saying GP ``settings'' say that is used for the ROM}
    ...
    \section{Results} \label{sec:Results}
        ...
        \subsection{Sine Function} \label{sub:Results:Sin}
            ...
            % \begin{table}[H]
            %     \centering
            %     \csvreader[tabular=|c|c|r|r|, 
            %     table head=\hline $N$ & $\sigma_{\epsilon}$ (\%) & \multicolumn{2}{c|}{$S_{\vr{1}}$ (\%)} \\\hline, 
            %     table foot=\hline, head=false]
            %     {results/sin.1.5.rom/amalgamated.S.csv}
            %     {}
            %     {\csvcolii & \csvcoli & \csvcoliii & \csvcolviii}
            %     \caption{\label{tab:Results:Sin:Sobol} Closed Sobol' indices calculated from initial (left sub-column) and optimized (right sub-column) GPs, for the sine function.}
            % \end{table}

            % \begin{table}[H]
            %     \centering
            %     \csvreader[tabular=|c|c|r|r|r|r|r|r|r|r|r|, 
            %     table head=\hline $N$ & $\sigma_{\epsilon}$ (\%) & \multicolumn{3}{c|}{$\sigma_{f(\textbf{x})}$ (\%)} & \multicolumn{3}{c|}{Outliers (\%)}  & \multicolumn{3}{c|}{RMSE (\%)} \\\hline, 
            %     table foot=\hline, head=false]
            %     {results/sin.1.5.rom/amalgamated.test_stats.csv}
            %     {}
            %     {\csvcolii & \csvcoli & \csvcoliv & \csvcolvii & \csvcolx & \csvcolv & \csvcolviii & \csvcolxi & \csvcoliii & \csvcolvi & \csvcolix}
            %     \caption{\label{tab:Results:Sin:testA} Predictive performance of initial GPs (left sub-columns) after optimizing the input basis (middle sub-columns) and reducing dimensionality (right sub-columns), for the sine function. Three measures are shown: the GPs' predictive standard deviation $\sigma_{f(\textbf{x})}$, the percentage of observations outside $\pm 2 \sigma_{f(\textbf{x})}$, and the Root Mean Square Error.}
            % \end{table}

            \begin{table}[H]
                \centering
                \csvreader[tabular=|c|c|r|r|r|, 
                table head=\hline $N$ & $\sigma_{\epsilon}$ (\%) & \multicolumn{3}{c|}{$S_{\vr{1}}$ (\%)} \\\hline, 
                table foot=\hline, head=false]
                {results/sin.1.5.random/amalgamated.amalgamated.S.csv}
                {}
                {\csvcolii & \csvcoli & \csvcolxv & \csvcolxx & \csvcolviii}
                \caption{\label{tab:Results:Sin:Sobol} Closed Sobol' indices calculated from initial (left sub-column) and optimized (middle sub-column) GPs, for the randomly rotated sine function. For comparison the Sobol' indices for the optimized GP on the unrotated sine function are shown in the rightmost sub-column.}
            \end{table}

            Turning immediately to the sine function with randomly rotated inputs, the first closed Sobol' index is shown in \cref{tab:Results:Sin:Sobol}. In all cases, the random rotation is effectively reversed within just 5 iterations of input basis optimization, as indicated by recovering virtually exact Sobol' indices. This is confirmed by the cross-validation predictive performance of the optimized GP, shown in \cref{tab:Results:Sin:testB}\todo{Is there not a nicer way to present these results? I can imagine a graph with outliers as a primary y-axis, RMSE as a secondary y-axis. Then 3 lines for each column and the x-axis being N}. It may be seen that the predictive uncertainty $\sigma_{f(\textbf{x})}$ closely replicates the noise $\sigma_{\epsilon}$ inherent in the test function, and the outliers are close to the 5\% expected of the normal distribution. Reducing dimensionality by fitting a GP to just the principal direction of the optimized basis barely affects the predictive performance.
            ...
            \begin{table}[H]
                ...
                table head=\hline $N$ & $\sigma_{\epsilon}$ (\%) & \multicolumn{3}{c|}{$\sigma_{f(\textbf{x})}$ (\%)} & \multicolumn{3}{c|}{Outliers (\%)}  & \multicolumn{3}{c|}{RMSE (\%)} \\\hline,
                table foot=\hline, head=false]
                {results/sin.1.5.random/amalgamated.test_stats.csv}
                {}
                {\csvcolii & \csvcoli & \csvcoliv & \csvcolvii & \csvcolx & \csvcolv & \csvcolviii & \csvcolxi & \csvcoliii & \csvcolvi & \csvcolix}
                \caption{\label{tab:Results:Sin:testB} Predictive performance of initial GPs (left sub-columns) after optimizing the input basis (middle sub-columns) and reducing dimensionality (right sub-columns), for the sine function with randomly rotated inputs. Three measures are shown: the GPs' predictive standard deviation $\sigma_{f(\textbf{x})}$, the percentage of observations outside $\pm 2 \sigma_{f(\textbf{x})}$, and the Root Mean Square Error.}
            \end{table}

            As expected, the active subspace measures $\Sigma_1(\vr{u^{\dag}})$ are all close to 100\%, confirming that the random rotation has been reversed.
            ...
            \begin{table}[H]
                ...
                table foot=\hline, head=false]
                {results/sin.1.5.random/amalgamated.Theta_Analyzed.csv}
        ...
        \subsection{Decoupled Ishigami Function} \label{sub:Results:Decoup}
            \begin{equation} \label{def:Decoup}
            ...
            \end{gather*}
            The predictive performance is shown in \cref{tab:Results:Decoup:testA}.

            \begin{table}[H]
                \centering
                \csvreader[tabular=|c|c|r|r|r|r|, 
                table head=\hline $N$ & $\sigma_{\epsilon}$ (\%) & \multicolumn{2}{c|}{$S_{\vr{1}}$ (\%)} & \multicolumn{2}{c|}{$S_{\vr{2}}$ (\%)} \\\hline, 
                table foot=\hline, head=false]
                {results/sin.2.5.rom/amalgamated.S.csv}
                {}
                {\csvcolii & \csvcoli & \csvcoliii & \csvcolviii & \csvcoliv & \csvcolix}
                \caption{\label{tab:Results:Decoup:Sobol} Closed Sobol' indices calculated from initial (left sub-column) and optimized (right sub-column) GPs, for the decoupled Ishigami function.}
            \end{table}
            ...
            \begin{table}[H]
                ...
                table head=\hline $N$ & $\sigma_{\epsilon}$ (\%) & \multicolumn{3}{c|}{$\sigma_{f(\textbf{x})}$ (\%)} & \multicolumn{3}{c|}{Outliers (\%)}  & \multicolumn{3}{c|}{RMSE (\%)} \\\hline, 
                table foot=\hline, head=false]
                {results/sin.2.5.rom/amalgamated.test_stats.csv}
                {}
                {\csvcolii & \csvcoli & \csvcoliv & \csvcolvii & \csvcolx & \csvcolv & \csvcolviii & \csvcolxi & \csvcoliii & \csvcolvi & \csvcolix}
                \caption{\label{tab:Results:Decoup:testA} Predictive performance of initial GPs (left sub-columns) after optimizing the input basis (middle sub-columns) and reducing dimensionality (right sub-columns), for the decoupled Ishigami function. Three measures are shown: the GPs' predictive standard deviation $\sigma_{f(\textbf{x})}$, the percentage of observations outside $\pm 2 \sigma_{f(\textbf{x})}$, and the Root Mean Square Error.}
            \end{table}

            \begin{table}[H]
                \centering
                \csvreader[tabular=|c|c|r|r|r|r|r|r|, 
                table head=\hline $N$ & $\sigma_{\epsilon}$ (\%) & \multicolumn{3}{c|}{$S_{\vr{1}}$ (\%)} & \multicolumn{3}{c|}{$S_{\vr{2}}$ (\%)} \\\hline, 
                table foot=\hline, head=false]
                {results/sin.2.5.random/amalgamated.amalgamated.S.csv}
                {}
                {\csvcolii & \csvcoli & \csvcolxvii & \csvcolxxii & \csvcolviii & \csvcolxviii & \csvcolxxiii & \csvcolix}
                \caption{\label{tab:Results:Decoup:Sobol} Closed Sobol' indices calculated from initial (left sub-column) and optimized (middle sub-column) GPs, for the randomly rotated decoupled Ishigami function. For comparison the Sobol' indices for the optimized GP on the unrotated Ishigami function are shown in the rightmost sub-column.}
            \end{table}
            ...
            \begin{table}[H]
                ...
                table head=\hline $N$ & $\sigma_{\epsilon}$ (\%) & \multicolumn{3}{c|}{$\sigma_{f(\textbf{x})}$ (\%)} & \multicolumn{3}{c|}{Outliers (\%)}  & \multicolumn{3}{c|}{RMSE (\%)} \\\hline,
                table foot=\hline, head=false]
                {results/sin.2.5.random/amalgamated.test_stats.csv}
                {}
                {\csvcolii & \csvcoli & \csvcoliv & \csvcolvii & \csvcolx & \csvcolv & \csvcolviii & \csvcolxi & \csvcoliii & \csvcolvi & \csvcolix}
                \caption{\label{tab:Results:Decoup:testB} Predictive performance of initial GPs (left sub-columns) after optimizing the input basis (middle sub-columns) and reducing dimensionality (right sub-columns), for the decoupled Ishigami function with randomly rotated inputs. Three measures are shown: the GPs' predictive standard deviation $\sigma_{f(\textbf{x})}$, the percentage of observations outside $\pm 2 \sigma_{f(\textbf{x})}$, and the Root Mean Square Error.}
            \end{table}

            \begin{table}[H]
                \centering
                \csvreader[tabular=|c|r|r|r|, 
                table head=\hline $N$ & $\sigma_{\epsilon}$ (\%)  & $\Sigma_1(\vr{u^{\dag}})$  & $\Sigma_2(\vr{u^{\dag}})$ \\\hline, 
                table foot=\hline, head=false]
                {results/sin.2.5.random/amalgamated.Theta_Analyzed.csv}
                {}{\csvcolii & \csvcoli & \csvcoliii & \csvcoliv}                
        ...
        %         \centering
        %         \csvreader[tabular=|c|c|r|r|r|r|r|r|r|r|r|, 
        %         table head=\hline $N$ & $\sigma_{\epsilon}$ (\%) & \multicolumn{3}{c|}{RMSE (\%)} & \multicolumn{3}{c|}{$\sigma_{f(\textbf{x})}$ (\%)} & \multicolumn{3}{c|}{Outliers (\%)} \\\hline, 
        %         table foot=\hline, head=false]
        %         {results/sin.2.5.rom/amalgamated.test_stats.csv}
        %         {}
        %         {\csvcolii & \csvcoli & \csvcoliii & \csvcolvi & \csvcolix & \csvcoliv & \csvcolvii & \csvcolx & \csvcolv & \csvcolviii & \csvcolxi}        
        ...
        \subsection{Ishigami Function} \label{sub:Results:Ishigami}
            ...
            \begin{table}[H]
                \centering
                \csvreader[tabular=|c|c|r|r|r|r|r|r|, 
                table head=\hline $N$ & $\sigma_{\epsilon}$ (\%) & \multicolumn{2}{c|}{$S_{\vr{1}}$ (\%)} & \multicolumn{2}{c|}{$S_{\vr{2}}$ (\%)} & \multicolumn{2}{c|}{$S_{\vr{3}}$ (\%)} \\\hline, 
                table foot=\hline, head=false]
                {results/ishigami.5.rom/amalgamated.S.csv}
                {}
                {\csvcolii & \csvcoli & \csvcoliii & \csvcolviii & \csvcoliv & \csvcolix & \csvcolv & \csvcolx}
                \caption{\label{tab:Results:Ishigami:Sobol} Closed Sobol' indices calculated from initial (left sub-column) and optimized (right sub-column) GPs, for the Ishigami function.}
            \end{table}
            ...
            \begin{table}[H]
                ...
                table head=\hline $N$ & $\sigma_{\epsilon}$ (\%) & \multicolumn{3}{c|}{$\sigma_{f(\textbf{x})}$ (\%)} & \multicolumn{3}{c|}{Outliers (\%)}  & \multicolumn{3}{c|}{RMSE (\%)} \\\hline, 
                table foot=\hline, head=false]
                {results/ishigami.5.rom/amalgamated.test_stats.csv}
                {}
                {\csvcolii & \csvcoli & \csvcoliv & \csvcolvii & \csvcolx & \csvcolv & \csvcolviii & \csvcolxi & \csvcoliii & \csvcolvi & \csvcolix}
                \caption{\label{tab:Results:Ishigami:testA} Predictive performance of initial GPs (left sub-columns) after optimizing the input basis (middle sub-columns) and reducing dimensionality (right sub-columns), for the Ishigami function. Three measures are shown: the GPs' predictive standard deviation $\sigma_{f(\textbf{x})}$, the percentage of observations outside $\pm 2 \sigma_{f(\textbf{x})}$, and the Root Mean Square Error.}
            \end{table}

            \begin{table}[H]
                \centering
                \csvreader[tabular=|c|c|r|r|r|r|r|r|r|r|r|r|r|, 
                table head=\hline $N$ & $\sigma_{\epsilon}$ (\%) & \multicolumn{3}{c|}{$S_{\vr{1}}$ (\%)} & \multicolumn{3}{c|}{$S_{\vr{2}}$ (\%)} & \multicolumn{3}{c|}{$S_{\vr{3}}$ (\%)} \\\hline, 
                table foot=\hline, head=false]
                {results/ishigami.5.random/amalgamated.amalgamated.S.csv}
                {}
                {\csvcolii & \csvcoli & \csvcolxix & \csvcolxxiv & \csvcolviii & \csvcolxx & \csvcolxxv & \csvcolix & \csvcolxxi & \csvcolxxvi & \csvcolx}
                \caption{\label{tab:Results:Ishigami:Sobol} Closed Sobol' indices calculated from initial (left sub-column) and optimized (middle sub-column) GPs, for the randomly rotated Ishigami function. For comparison the Sobol' indices for the optimized GP on the unrotated Ishigami function are shown in the rightmost sub-column.}
            \end{table}
            ...
            \begin{table}[H]
                ...
                table head=\hline $N$ & $\sigma_{\epsilon}$ (\%) & \multicolumn{3}{c|}{$\sigma_{f(\textbf{x})}$ (\%)} & \multicolumn{3}{c|}{Outliers (\%)}  & \multicolumn{3}{c|}{RMSE (\%)} \\\hline,
                table foot=\hline, head=false]
                {results/ishigami.5.random/amalgamated.test_stats.csv}
                {}
                {\csvcolii & \csvcoli & \csvcoliv & \csvcolvii & \csvcolx & \csvcolv & \csvcolviii & \csvcolxi & \csvcoliii & \csvcolvi & \csvcolix}
                \caption{\label{tab:Results:Ishigami:testB} Predictive performance of initial GPs (left sub-columns) after optimizing the input basis (middle sub-columns) and reducing dimensionality (right sub-columns), for the Ishigami function with randomly rotated inputs. Three measures are shown: the GPs' predictive standard deviation $\sigma_{f(\textbf{x})}$, the percentage of observations outside $\pm 2 \sigma_{f(\textbf{x})}$, and the Root Mean Square Error.}
            \end{table}

            \begin{table}[H]
                \centering
                \csvreader[tabular=|c|r|r|r|r|, 
                table head=\hline $N$ & $\sigma_{\epsilon}$ (\%) & $\Sigma_1(\vr{u^{\dag}})$ & $\Sigma_2(\vr{u^{\dag}})$ & $\Sigma_3(\vr{u^{\dag}})$ \\\hline, 
                table foot=\hline, head=false]
                {results/ishigami.5.random/amalgamated.Theta_Analyzed.csv}
                {}{\csvcolii & \csvcoli & \csvcoliii & \csvcoliv & \csvcolv}                
        ...
        \subsection{Sobol' G Function} \label{sub:Results:sobol_g}
            ...
            \begin{gather*}
                \vr{a}_{i} = (i-1)/2 \\
                S_{\vr{1}} = 0.3575 \QT{;}S_{\vr{2}} = 0.8342 \QT{;} S_{\vr{3}} = 0.9930 \QT{;} S_{\vr{4}} = 0.9999 \QT{;} S_{\vr{5}} = 1
            \end{gather*}
        ...
        \begin{table}[H]
            ...
            table head=\hline $N$ & $\sigma_{\epsilon}$ (\%) & \multicolumn{2}{c|}{$S_{\vr{1}}$ (\%)} & \multicolumn{2}{c|}{$S_{\vr{2}}$ (\%)} & \multicolumn{2}{c|}{$S_{\vr{3}}$ (\%)} \\\hline, 
            table foot=\hline, head=false]
            {results/sobol_g.5.rom/amalgamated.S.csv}
            {}
            {\csvcolii & \csvcoli & \csvcoliii & \csvcolviii & \csvcoliv & \csvcolix & \csvcolv & \csvcolx}
            \caption{\label{tab:Results:SobolG:Sobol} Closed Sobol' indices calculated from initial (left sub-column) and optimized (right sub-column) GPs, for the Sobol' G function.}
        \end{table}
        ...
        \begin{table}[H]
            ...
            table head=\hline $N$ & $\sigma_{\epsilon}$ (\%) & \multicolumn{3}{c|}{$\sigma_{f(\textbf{x})}$ (\%)} & \multicolumn{3}{c|}{Outliers (\%)}  & \multicolumn{3}{c|}{RMSE (\%)} \\\hline, 
            table foot=\hline, head=false]
            {results/sobol_g.5.rom/amalgamated.test_stats.csv}
            {}
            {\csvcolii & \csvcoli & \csvcoliv & \csvcolvii & \csvcolx & \csvcolv & \csvcolviii & \csvcolxi & \csvcoliii & \csvcolvi & \csvcolix}
            \caption{\label{tab:Results:SobolG:testA} Predictive performance of initial GPs (left sub-columns) after optimizing the input basis (middle sub-columns) and reducing dimensionality (right sub-columns), for the Sobol' G function. Three measures are shown: the GPs' predictive standard deviation $\sigma_{f(\textbf{x})}$, the percentage of observations outside $\pm 2 \sigma_{f(\textbf{x})}$, and the Root Mean Square Error.}
        \end{table}
        ...
        \begin{table}[H]
            ...
            table head=\hline $N$ & $\sigma_{\epsilon}$ (\%) & \multicolumn{3}{c|}{$S_{\vr{1}}$ (\%)} & \multicolumn{3}{c|}{$S_{\vr{2}}$ (\%)} & \multicolumn{3}{c|}{$S_{\vr{3}}$ (\%)} \\\hline, 
            table foot=\hline, head=false]
            {results/sobol_g.5.random/amalgamated.amalgamated.S.csv}
            {}
            {\csvcolii & \csvcoli & \csvcolxx & \csvcolxxv & \csvcolviii & \csvcolxxi & \csvcolxxvi & \csvcolix & \csvcolxxii & \csvcolxxvii & \csvcolx}
            \caption{\label{tab:Results:SobolG:Sobol} Closed Sobol' indices calculated from initial (left sub-column) and optimized (middle sub-column) GPs, for the randomly rotated Sobol' G function. For comparison the Sobol' indices for the optimized GP on the unrotated Sobol' G function are shown in the rightmost sub-column.}
        \end{table}
        ...
        \begin{table}[H]
            ...
            table head=\hline $N$ & $\sigma_{\epsilon}$ (\%) & \multicolumn{3}{c|}{$\sigma_{f(\textbf{x})}$ (\%)} & \multicolumn{3}{c|}{Outliers (\%)}  & \multicolumn{3}{c|}{RMSE (\%)} \\\hline,
            table foot=\hline, head=false]
            {results/sobol_g.5.random/amalgamated.test_stats.csv}
            {}
            {\csvcolii & \csvcoli & \csvcoliv & \csvcolvii & \csvcolx & \csvcolv & \csvcolviii & \csvcolxi & \csvcoliii & \csvcolvi & \csvcolix}
            \caption{\label{tab:Results:SobolG:testB} Predictive performance of initial GPs (left sub-columns) after optimizing the input basis (middle sub-columns) and reducing dimensionality (right sub-columns), for the Sobol' G function with randomly rotated inputs. Three measures are shown: the GPs' predictive standard deviation $\sigma_{f(\textbf{x})}$, the percentage of observations outside $\pm 2 \sigma_{f(\textbf{x})}$, and the Root Mean Square Error.}
        \end{table}
        ...
        \begin{table}[H]
            ...
            table head=\hline $N$ & $\sigma_{\epsilon}$ (\%) & $\Sigma_1(\vr{u^{\dag}})$ & $\Sigma_2(\vr{u^{\dag}})$ & $\Sigma_3(\vr{u^{\dag}})$ \\\hline, 
            table foot=\hline, head=false]
            {results/sobol_g.5.random/amalgamated.Theta_Analyzed.csv}
            {}{\csvcolii & \csvcoli & \csvcoliii & \csvcoliv & \csvcolv}                
            \caption{\label{tab:Results:SobolG:Theta} The active subspace measures $\Sigma_m(\vr{u^{\dag}})$ for the Sobol' G function, comparing optimization of unrotated and randomly rotated inputs.} 
        \end{table}
