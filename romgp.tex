...
\begin{document}
    ...
    \section{Methodology} \label{sec:Method}
        ...
        \subsection{Summary} \label{sub:Method:Summary}
            ...
            \begin{algorithm}
                \begin{algorithmic}[1]
                    \REPEAT
                        \STATE Fit GP surrogate to $y(\vr{X})$, determining $\bar{f}(\vr{x})$ according to \cref{sub:Method:GP}
                        \STATE Set $\tilde{\Theta} \leftarrow \Theta \leftarrow \vr{I}$
                        \FOR{$m=1$ \TO $M$}
                            ...
                            \STATE  Reset $\tilde{\Theta} \leftarrow \vr{Q}^{\intercal}$ where $\Theta^{\intercal} = \vr{Q}\vr{R}$
                        \ENDFOR
                    \STATE Reset the input basis to $\vr{X} = \vr{X} \Theta^{\intercal}$
    ...
    \section{Results} \label{sec:Results}
        The algorithm described in \cref{sub:Method:Summary} has been tested on a suite of test functions.
        In each case an \M{N \times M} design matrix \vr{X} is sampled from a standard normal distribution. 
        The input to the test function \M{f: \lbrack x_-, x_+ \rbrack^{M} \rightarrow \st{R}} is generally constructed as
        \begin{equation} \label{def:Xhat}
            \vr{\hat{X}}^{\intercal} = (x_+ - x_-) c(\Phi X^{\intercal}) + x_-(\vr{I})_{\vr{M} \times \vr{N}}
        \end{equation}
        where \M{c} is the cumulative density function for independent standard normal random variables, and \M{\Phi} is a test rotation matrix which is either identity or random.
        ...
        \subsection{Ishigami Function} \label{sub:Results:Ishigami}
            ...
            \begin{gather*}
                a = 7.0 \QT{;} b = 0.1 \\
                S_{\vr{1}} \deq S_{(1)} = 0.313905 \QT{;}S_{(2)} = 0.442411 \QT{;} S_{(3)} = 0 \\
                S_{\vr{2}} \deq S_{(1,2)} = 0.756316 \QT{;} S_{\vr{3}} \deq S_{(1,2,3)} = 1
            \end{gather*}
