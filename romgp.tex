            ...
            \begin{algorithm}
                \begin{algorithmic}[1]
                    \REPEAT
                        ...
                        \STATE Set $\hat{\Theta} \leftarrow \Theta \leftarrow \vr{I}$
                        \FOR{$m=1$ \TO $M$}
                            \STATE According to \cref{sub:Method:GSA}, optimize
                            $$(\Xi)_{m\times\vr{M}\setminus\vr{m}} \leftarrow \argmax_{(\Xi)_{m\times\vr{M}\setminus\vr{m}}} D_{\vr{m}}((\Theta)_{\vr{m}\times\vr{M}})$$
                            where $(\Theta)_{\vr{m}\times\vr{M}} \deqr (\Xi)_{\vr{m}\times\vr{M}} \hat{\Theta}$
                            \STATE  Reset $\hat{\Theta} \leftarrow \vr{Q}^{\intercal}$ where $\Theta^{\intercal} = \vr{Q}\vr{R}$
                        \ENDFOR
